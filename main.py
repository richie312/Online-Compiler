...
def api_performance():
    ...
    #return "<h3 style=color:red;font-family:verdana;>API Performance Page in Maintenance</h3>"
    #return redirect('http://111.93.164.108:5002/', code=302)
...
def get_problems():
    ...
    try:
        if level == "All":
            ...
            level = [data[i][12] for i in range(len(data))]
            response = []
            for i in range(len(problem_id)):
                response.append({'problem_id': problem_id[i], 
                          'level' : level[i],
                          'problem_code':problem_code[i],
...
def submission_id():
    ...
    try:
        ...
        cursor.execute("""select language from sphere_engine_languages;""")
        all_lang = cursor.fetchall()
        response = Response(json.dumps("Please enter the correct language. Avalialable languages are {}".format(all_lang)))
        response.status_code = 400
    
    cursor.execute("""select id from sphere_engine_problem_details where name = %s;""", (problem_name,))
    problem_id = cursor.fetchone()[0]
    """get the required pckgs for the checkfile for specific language"""
    cursor.execute("""select required_pckgs from sphere_engine_testcase where problem_id=%s;""", (problem_id,))
    required_pckgs = cursor.fetchall()[0][0]
    required_pckgs = eval(required_pckgs)
    # get required package for specific language
    try:
        opening_statement = required_pckgs[lang]
    except KeyError:
        opening_statement = "\n"
    ...
    try:
        ...
        submission_file = opening_statement + source + check_file
    except KeyError:
        None
    """ Submission file """
    try:
        try:
            ...
            response = json.dumps({"submissionId": response['id']})
...
def update_problem():
    ...
    for i in range(len(list_sphere_keys)):
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET id=%s where id=%s;',(response['id'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET code=%s where id=%s;',(response['code'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET name=%s where id=%s;',(response['name'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET shortBody=%s where id=%s;',(response['shortBody'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET body=%s where id=%s;',(response['body'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET type=%s where id=%s;',(json.dumps(response['type']),data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET interactive=%s where id=%s;',(response['interactive'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET masterjudge=%s where id=%s;',(response['masterjudge']['id'],data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET testcases=%s where id=%s;',(json.dumps(response['testcases']),data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET lastModified=%s where id=%s;',(json.dumps(response['lastModified']),data['id'],))
        connection.commit()
        cursor.execute('UPDATE skillz.sphere_engine_problem_details SET permissions=%s where id=%s;',(json.dumps(response['permissions']),data['id'],))
        connection.commit()
    cursor.close()
    connection.close()
...
def get_testcases():
    ...
    try:
        ...
        cursor.execute("select check_file,required_pckgs,test_case_dict from sphere_engine_testcase where problem_id=%s",(problemId,))
        data = cursor.fetchall()
        check_file = eval(data[0][0])
        testcase_details = data[0][2]
        required_pckgs = eval(data[0][1])
        if language not in required_pckgs.keys():
            response = {"check_file": check_file[language],
                    "testcases": testcase_details,
                    "required_pckgs": "None"}
        else:
            response = {"check_file": check_file[language],
                    "testcases": testcase_details,
                    "required_pckgs": required_pckgs[language]}
    
    except IndexError:
        response = {"response":"Language does not exist."}    
    return jsonify(response)
            
@app.route('/update_checkfile_required_packages',methods = ['PUT'])
def udpate_packages():
    connection = mysql.connector.connect(host=database_auth['DB Host'], user=database_auth['DB Username'],port=3306,
               passwd=database_auth['DB Password'], db=database_auth['Database'])
    cursor = connection.cursor()                
    data = request.get_json()
    try: 
        cursor.execute("""select required_pckgs from sphere_engine_testcase where problem_id=%s""",(data['problem_id'],))
        required_pckgs = cursor.fetchall()    
        if required_pckgs[0][0] == None:
            required_pckgs_dict = {}
            required_pckgs_dict.update({data['language']:data['required_pckgs']})
        else:
            required_pckgs_dict = eval(required_pckgs[0][0])
            required_pckgs_dict.update({data['language']:data['required_pckgs']})    
        cursor.execute("""Update sphere_engine_testcase set required_pckgs=%s where problem_id=%s""",(json.dumps(required_pckgs_dict),data['problem_id'],))
        connection.commit()
...
@app.route('/get_checkfile_required_packages',methods = ['GET'])
def get_packages():
    connection = mysql.connector.connect(host=database_auth['DB Host'], user=database_auth['DB Username'],port=3306,
               passwd=database_auth['DB Password'], db=database_auth['Database'])
    cursor = connection.cursor()                
    query_parameters = request.args
    problemId = query_parameters.get('problemId',type = int)
    language = query_parameters.get('language')
    cursor.execute("""select required_pckgs from sphere_engine_testcase where problem_id=%s""",(int(problemId),))
    required_pckgs = cursor.fetchall()
    if language not in eval(required_pckgs[0][0]).keys():
        response = {"Required_Packages for {}".format(language): "Not Required"}
    else:
        required_pckgs_dict = eval(required_pckgs[0][0])
        response = {"required_pckgs for {}".format(language):required_pckgs_dict[language]}
    cursor.close()
    connection.close()
    return jsonify(response)
    
@app.route('/delete_checkfile_required_packages',methods = ['DELETE'])
def delete_requirements():
    ...
    language = data['language']
    cursor.execute("""select required_pckgs from sphere_engine_testcase where problem_id=%s""",(problemId,))
    required_pckgs = cursor.fetchall()
    if language not in eval(required_pckgs[0][0]).keys():
        response = {"Required_Packages for {}".format(language): "Language Does not exist"}
    else:
        required_pckgs_dict = eval(required_pckgs[0][0])
        try:    
            del required_pckgs_dict[language]
            response = {"Response": "Deleted Successfully."}
            cursor.execute("""UPDATE sphere_engine_testcase set required_pckgs=%s where problem_id=%s""",(json.dumps(required_pckgs_dict),problemId,))
            connection.commit()
            cursor.close()
            connection.close()
        except KeyError:
            response = {"Response": "No Such Language exist in database."} 
...
def get_languages():
    ...
    data = cursor.fetchall()
    data_dict = {"Languages":"Compiler_Id"}
    for i in range(len(data)):
